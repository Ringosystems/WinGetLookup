...
.NOTES
    ...
    VERSION HISTORY:
    2.0.0 - Major refactor for WinGet schema fidelity (STABLE RELEASE):
...
#region Private Functions - Utility

function Get-PropertyValue {
    <#
    .SYNOPSIS
        Safely gets a property value from either a PSObject or Hashtable.
    .DESCRIPTION
        Works with both PSObject (PowerShell 5.1 Invoke-RestMethod) and 
        Hashtable (PowerShell 7+ with -AsHashtable).
    #>
    param(
        [Parameter(Mandatory)]
        $Object,
...
function Invoke-WinGetApiWithRetry {
    ...
    while ($attempt -lt $MaxRetries) {
        ...
        try {
            ...
            # Use Invoke-WebRequest + ConvertFrom-Json -AsHashtable to avoid case sensitivity issues
...
function Get-CachedApiResponse {
    ...
    try {
        ...
        # Parse response - always hashtable in PS7+
        $packages = $null
...
function Get-BestPackageMatch {
    ...
    # Use @() to ensure array even with single result to avoid string indexing issues
